...
import { Trash2, Star, GripVertical } from 'lucide-react-native';
...
export default function DraggableItemCard({ 
  ...
  // Create unique IDs for gesture handlers with type prefix to prevent conflicts
  const typePrefix = isBook ? 'book' : 'movie';
  const dragHandlerId = `${typePrefix}-drag-${item.id || index}`;
  const swipeHandlerId = `${typePrefix}-swipe-${item.id || index}`;
  ...
  const getAccessibilityLabel = () => {
    ...
    const reorderHint = canReorder ? '. This item can be reordered by dragging' : '';
    
    return `${position} ${itemType}: ${item.title} by ${item.author}${rating}${progress}${format}${completedDate}${allTime}${notes}${reorderHint}`;
  };

  const getAccessibilityHint = () => {
    if (Platform.OS === 'web') {
      return canReorder 
        ? 'Tap to edit, or use the delete button to remove this item. This item can be reordered.'
        : 'Tap to edit, or use the delete button to remove this item';
    }
    return canReorder 
      ? 'Tap to edit. Use the drag handle to reorder, or swipe left to reveal delete option'
      : 'Tap to edit. Swipe left to reveal delete option, or swipe far left to delete immediately';
  };

  // Swipe gesture for delete - ALWAYS ENABLED
  ...
  // Drag gesture for reordering - only when canReorder is true
  const onDragGestureEvent = Animated.event(
    [{ nativeEvent: { translationY: translateY } }],
    { useNativeDriver: true }
  );

  const onDragHandlerStateChange = (event: any) => {
    if (event.nativeEvent.state === State.BEGAN) {
      ...
      // Announce drag start to screen reader
      AccessibilityInfo.announceForAccessibility(`Started dragging ${item.title}. Move up or down to reorder.`);
    } else if (event.nativeEvent.oldState === State.ACTIVE) {
      const { translationY } = event.nativeEvent;
      const itemHeight = 80; // Approximate item height
      const newIndex = Math.max(0, index + Math.round(translationY / itemHeight));
      
      onDragEnd?.(index, newIndex);
      
      // Announce drag end to screen reader
      if (newIndex !== index) {
        AccessibilityInfo.announceForAccessibility(`${item.title} moved to position ${newIndex + 1}`);
      } else {
        AccessibilityInfo.announceForAccessibility(`${item.title} returned to original position`);
      }
  ...
  // For web platform, use simple touch handlers
  if (Platform.OS === 'web') {
    return (
      <View style={[
        styles.container, 
          ...
          <View style={styles.content}>
            {canReorder && (
              <View 
                ...
                accessibilityHint="Use this to reorder the item in the list"
              >
                <GripVertical size={16} color={isDark ? '#6B7280' : '#9CA3AF'} />
              </View>
            )}
  ...
  return (
    <Animated.View style={[
      ...
      {
        transform: [
          { translateY: dragOffset || translateY },
          { scale }
        ]
      }
    ]}>
      {/* Delete button background - ALWAYS PRESENT */}
      <Animated.View 
        style={[
      ...
      {/* Main card content with swipe gesture - ALWAYS ENABLED */}
      <PanGestureHandler
        key={swipeHandlerId}
        id={swipeHandlerId}
        ...
        activeOffsetX={[-10, 10]}
        failOffsetY={[-20, 20]}
        simultaneousHandlers={canReorder ? [dragHandlerId] : undefined}
      >
        <Animated.View 
          style={[
            styles.cardContent,
        ...
        >
          {canReorder ? (
            // Draggable version with drag handle AND swipe to delete
