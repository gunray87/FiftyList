/** Max stored/displayed taste snapshot length (keep in sync with LLM proxy). */
export const TASTE_NARRATIVE_MAX_CHARS = 1080;

/** Max taste narrative sent with suggestions-refine (card-copy context). */
export const TASTE_NARRATIVE_REFINE_MAX_CHARS = 320;

const ENDS_WITH_SENTENCE = /[.!?]["')\]]?\s*$/;

/** Index in `text` immediately after the last sentence-ending punctuation. */
...
/**
 * Keep only complete sentences within `max` characters.
 * Drops a trailing fragment that does not end with sentence punctuation.
 */
...
/** One or two sentences about films, built from list data (not LLM guesswork). */
...
/**
 * Ensures the taste snapshot names real films from the user's list when they watch movies.
 */
